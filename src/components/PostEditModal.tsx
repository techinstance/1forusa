...
interface PostEditModalProps {
  ...
  post: Post | null;
  onClose: () => void;
  onSave: (postId: string, text: string, attachments: Asset[]) => void;
}
...
const PostEditModal: React.FC<PostEditModalProps> = ({
  ...
  const handleSave = () => {
    ...
    if (postText.trim() || attachments.length > 0) {
      onSave(post.id, postText.trim(), attachments);
      onClose();
    }
  };
  ...
  const hasChanges = () => {
    ...
    return postText !== post.text || 
           attachments.length !== post.attachments.length ||
           attachments.some((att, index) => att.uri !== post.attachments[index]?.uri);
  };
          ...
          >
            ...
            <View style={styles.header}>
              ...
              <TouchableOpacity
                ...
                style={[
                  styles.saveButton,
                  (!hasChanges() || (!postText.trim() && attachments.length === 0)) && styles.disabledButton,
                ]}
                disabled={!hasChanges() || (!postText.trim() && attachments.length === 0)}
              >
